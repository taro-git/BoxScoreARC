...
from rest_api.serializers.game_summary import GameSummarySerializer, TeamSerializer, GameSummaryCreate
from rest_api.utils.fetch_player_on_game import fetch_player_on_game

##
## Fetch from nba_api
#### 
def fetch_game_summaries_by_season(season:str=None) -> List[GameSummaryCreate]:
    ...
    try:
        schedule_league = scheduleleaguev2.ScheduleLeagueV2(season=season) if season else scheduleleaguev2.ScheduleLeagueV2()
    except:
...
def fetch_game_summaries_by_date(date_est: datetime | None) -> List[GameSummaryCreate]:
    ...
    for game_id in game_ids:
        ...
        home_team_line_score = filtered_line_score[filtered_line_score["TEAM_ID"] == filtered_game_header["HOME_TEAM_ID"].iloc[0]]
        away_team_line_score = filtered_line_score[filtered_line_score["TEAM_ID"] != filtered_game_header["HOME_TEAM_ID"].iloc[0]]
...
def update_players_in_game_summary_by_game_id(game_id:str) -> GameSummaryCreate:
    """game_id を指定して、既存の GameSummary モデルに対して players 情報を更新した GameSummaryCreate クラスを作成します.  
...
def upsert_game_summary(game_summary_create: GameSummaryCreate):
    ...
    _create_not_existing_team(game_summary_create.get('home_team_id', -1), game_summary_create.pop('home_team_abb', 'unknown'))
