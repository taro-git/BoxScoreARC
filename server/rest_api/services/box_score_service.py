...
from nba_api.stats.endpoints import boxscoretraditionalv3, playbyplayv2
...
from rest_api.serializers.box_score import BoxScoreCreate, BoxScoreSerializer
from rest_api.utils.fetch_player_on_game import PlayersDict, fetch_player_on_game
        ...
        match one_play.EVENTMSGTYPE:
            ...
            case 2:
                box_score_create_maker.append_box_score_data(
                    ...
                    [
                        1,
                        1 if _contains_3pt(one_play.HOMEDESCRIPTION, one_play.VISITORDESCRIPTION) else 0,
                    ],
                )
                ...
                for player in players:
                    if player["player_id"] == one_play.PLAYER1_ID:
                        ...
                        box_score_create_maker.append_box_score_data(
                            ...
                            [
                                1,
                                comulative_reb["def"] - last_dreb,
                                comulative_reb["off"] - last_oreb,
                            ],
                        )
            # ターンオーバー
            case 5:
                box_score_create_maker.append_box_score_data(
                    elapsed_seconds, one_play.PLAYER1_ID, is_home_player, ["to"], [1]
                )
                if one_play.PLAYER2_ID != 0:
                    box_score_create_maker.append_box_score_data(
                        elapsed_seconds,
            ...
            case 6:
                box_score_create_maker.append_box_score_data(
                    elapsed_seconds, one_play.PLAYER1_ID, is_home_player, ["pf"], [1]
                )
            ...
            # 選手交代
            case 8:
                box_score_create_maker.players_to_bench(
                    last_elapsed_seconds,
                    elapsed_seconds,
                    [one_play.PLAYER1_ID],
                    is_home_player,
                )
                box_score_create_maker.players_to_court(
                    last_elapsed_seconds,
                    elapsed_seconds,
                    [one_play.PLAYER2_ID],
                    is_home_player,
...
def _update_on_court_player_id(
    ...
    try:
        ...
        if len(home_player_stats) == 5:
            ...
            home_player_id = home_player_stats[home_player_stats["personId"].isin(on_court_home_player_id)][
                "personId"
            ].tolist()
        ...
        if len(away_player_stats) == 5:
            ...
            away_player_id = away_player_stats[away_player_stats["personId"].isin(on_court_away_player_id)][
                "personId"
            ].tolist()
