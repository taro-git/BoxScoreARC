...
def _make_box_score_create(box_score: BoxScore, game_summary: GameSummary) -> BoxScoreCreate:
    ...
    def _update_box_score_data_values(is_home: bool, plyaers_stats: dict, team_stats: dict) -> None:
        for player in plyaers_stats:
            box_score_create_maker.append_box_score_data(0, player['personId'], is_home, keys, [
        ...
        box_score_create_maker.append_box_score_data(0, team_stats['teamId'], is_home, keys, [
            0, 0, team_stats['statistics']['reboundsTeam'], 0, 0, 0, 0, 0, 0, 0, 0, 0, 
            team_stats['statistics']['reboundsTeamOffensive'], team_stats['statistics']['reboundsTeamDefensive'],
            team_stats['statistics']['turnoversTeam'], 0, 0
        ])
    
    _update_box_score_data_values(True, box_score.home_team_player_stats.get_dict(), box_score.home_team_stats.get_dict())
    _update_box_score_data_values(False, box_score.away_team_player_stats.get_dict(), box_score.away_team_stats.get_dict())
