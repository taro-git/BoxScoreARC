...
function hexToRgba(hex: RgbaColor): { r: number; g: number; b: number; a: number } {
    let raw = hex.slice(1)
...
function rgbaToHex({ r, g, b, a }: { r: number; g: number; b: number; a: number }): RgbaColor {
...
function convertColor(color: RgbaColor, ratio: number): RgbaColor {
    ...
    return rgbaToHex({ r: convertedR, g: convertedG, b: convertedB, a: convertedA })
}
