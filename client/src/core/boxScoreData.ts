import {
  ...
  type PlayerOnBoxScore,
} from "../types/BoxScore";

export const updateBoxScoreData = (
  ...
  return {
    boxScoreTableData: boxScoreTableData,
    homeStats: updateForTeamStats(homeStats).slice(1),
    awayStats: updateForTeamStats(awayStats).slice(1),
  };
};

const updatePlayerBoxScore = (
  player: PlayerOnBoxScore,
  startRange: number,
  endRange: number,
) => {
  ...
  if (!startBoxScore || !endBoxScore) {
    return calcShootingPercentageAdd(
      new Array(boxScoreRaw[0].length - 2).fill(0),
    );
  } else {
    return calcShootingPercentageAdd(
      endBoxScore.map((v, i) => Math.round((v - startBoxScore[i]) * 10) / 10),
    );
  }
};
...
const updateForTeamStats = (boxScoreRow: number[]) => {
  ...
  if (boxScoreRow[13] !== 0) {
    result[14] =
      Math.round((boxScoreRow[12] / boxScoreRow[13]) * 100 * 10) / 10;
  }
