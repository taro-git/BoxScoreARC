...
export class Cache<T> {
  ...
  constructor(options: CacheOptions<T> = {}) {
    ...
    this.getDistanceForFarthest =
      options.getDistanceForFarthest ??
      (() => {
        throw new Error(
          "getDistanceForFarthest was called, but no implementation was provided.",
        );
      });
  ...
  async getOrFetch(key: number, fetcher: () => Promise<T>): Promise<T> {
    ...
    if (
      this.cacheFilter === undefined ||
      (this.cacheFilter && this.cacheFilter(key, value))
    ) {
      this.cache.set(key, {
        value,
        timestamp: now,
