<template>
  <v-sheet class="bg-base hide-scroll d-flex flex-column fill-height" fluid>
    <v-tabs
      ...
      fixed-tabs
    >
      <v-tab v-for="tab of Tabs" :key="tab" :value="tab">{{
        tabTitles[tab]
      }}</v-tab>
    </v-tabs>
      ...
      >
        <v-sheet
          v-if="
            ...
            game.gameSummary.statusId == 3
          "
          class="bg-base px-6 text-center"
        >
          <v-select
            v-model="selectedQuarterRange"
            label="range of quarter"
            :items="quarterRangeLabels"
          />
          {{ formatReminTime(gameClockRange[0]) }} ~
          {{ formatReminTime(gameClockRange[1]) }}
          <v-range-slider
...
import { type Component, computed, type ComputedRef, ref, watch } from "vue";
...
import {
  quarterRangeLabels,
  type quarterRangeVariations,
} from "../types/QuarterRangeVariations";
...
const selectedTab = ref<Tabs>(Tabs.BoxScore);
const tabTitles: Record<Tabs, string> = {
  teamStats: "Team Stats",
  boxScore: "Box Score",
  headToHeadRecord: "Head to Head Record",
};
const tabComponents: Record<Tabs, Component> = {
  teamStats: TeamStats,
  boxScore: BoxScore,
  headToHeadRecord: HeadToHeadRecord,
};
const tabProps = computed<Record<Tabs, object>>(() => ({
  teamStats: {},
  boxScore: {
    gameId: gameId,
...
const selectedQuarterRange = ref<quarterRangeVariations>(
  settings.defaultQuarterRangeType,
);
const quaterRange: ComputedRef<
  Record<quarterRangeVariations, Record<"maxQuater" | "minQuater", number>>
> = computed(() => {
  return {
    ...
    OT: {
      maxQuater: game.finalPeriod <= 4 ? 5 : game.finalPeriod,
      minQuater: 5,
    },
  };
});
const maxSeconds = computed(() => {
  ...
  return maxQuarter <= 4
    ...
    : game.finalPeriod == 4
      ? 12 * 4 * 60
      : 12 * 4 * 60 + 5 * (maxQuarter - 4) * 60;
});
const minSeconds = computed(() => {
  const minQuarter = quaterRange.value[selectedQuarterRange.value].minQuater;
  return minQuarter <= 4
    ? 12 * (minQuarter - 1) * 60
    : 12 * 4 * 60 + 5 * (minQuarter - 5) * 60;
});
const gameClockRange = ref([
  minSeconds.value,
  settings.scoreDisplay ? maxSeconds.value : minSeconds.value,
]);
