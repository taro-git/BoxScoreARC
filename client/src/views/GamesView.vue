<template>
  <v-container class="d-flex flex-column h-100" fluid>
    ...
    <v-sheet
      class="d-flex flex-grow-1 overflow-hidden position-relative bg-base"
    ...
    >
      <div
        ref="scroll"
        class="d-flex flex-column hide-scrollbar"
        ...
        :style="{
          ...
          transition: isSliding
            ? `transform ${slideAnimationDurationSec}s ease`
            : 'none',
      ...
      >
        ...
        >
          <v-skeleton-loader
            elevation="7"
            :loading="gameSummariesSets.isLoadingMap.value[dayOffset]"
            type="image"
            class="w-100"
            color="lighten"
          >
        ...
        <v-card
          v-if="
            gameSummariesSets.gameSummaryMap.value[dayOffset].length === 0 &&
            !gameSummariesSets.error.value[dayOffset].isError
          "
          elevation="0"
          class="bg-base"
        >
          <v-card-title class="text-center text-h3 justify-center"
            >no game</v-card-title
          >
        </v-card>
        <v-empty-state
          v-if="gameSummariesSets.error.value[dayOffset].isError"
          title="Whoops, Server Error"
          :text="gameSummariesSets.error.value[dayOffset].errorMessage"
          class="text-accent"
        />
      </div>
    </v-sheet>
...
const gameSummariesSets = new GameSummariesSets(
...
const updategameSummariesSets = async () => {
  ...
  let targetDate: Date;
  const sortedDayOffsets = [...gameSummariesSets.dayOffsetsList].sort(
    (a, b) => {
      if (a === 0) return -1;
      if (b === 0) return 1;
      return a - b;
  ...
  for (const dayOffset of sortedDayOffsets) {
    ...
    try {
      const response =
        await gameSummariesApi.getGameSummariesByDate(targetDate);
      const sortedResponse = response.sort((a, b) => {
        if (a.statusId !== b.statusId) {
          return (
            statusIdPriority.indexOf(a.statusId) -
            statusIdPriority.indexOf(b.statusId)
          );
        }
...
const updateDate = (date: Date) => {
  const element = scroll.value?.[
    Math.floor(gameSummariesSets.dayOffsetsList.length / 2)
...
const {
  ...
  slideToNextDay,
  slideToPreviousDay,
  gameSummariesSets.dayOffsetsList.map((dayOffset) => {
