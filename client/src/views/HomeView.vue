<template>
    ...
    <v-data-table
        v-for="segment in tableSeparation == 'all' ? [''] : tableSeparation == 'conferences' ? conferences : divisions"
        :headers="headers"
...
import { type RegularSeasonTeamStats, SeasonSummary, conferences, divisions } from '../types/SeasonSummary';
...
seasonSummariesApi.getSeasonSummaries().then((response) => {
    seasonSummaries.value = response.sort((a, b) => b.season.localeCompare(a.season))
    seasons.value = seasonSummaries.value.map(seasonSummary => seasonSummary.season)
